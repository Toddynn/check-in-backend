...
@Injectable()
export class FindAllFeedbacksByEventIdAndAdminUseCase {
	...
	async execute(event_id: string, { page, quantity }: GenericPagination) {
		...
		const [feedbacks, count] = await this.feedbacksRepositoryInterface.findAllByEventIdAndAdmin(event_id, { page, quantity });
